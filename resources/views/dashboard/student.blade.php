...
<div class="max-w-7xl mx-auto">
    ...
    <div class="mb-8">
        ...
        <p class="text-emerald-700 font-medium mt-1">
            You are currently enrolled in {{ $courses->count() ?? 0 }} {{ Str::plural('course', $courses->count() ?? 0) }} this semester.
        </p>
    </div>
                ...
                <div class="p-3 bg-emerald-100 rounded-xl mr-4 text-emerald-600">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                              d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.247 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"></path>
                    </svg>
                </div>
                <div>
                    <p class="text-slate-400 text-xs font-bold uppercase tracking-wider">Enrolled</p>
                    <p class="text-2xl font-black text-slate-800">{{ $courses->count() }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-2xl shadow-xl shadow-emerald-900/10 border-b-4 border-blue-500">
            <div class="flex items-center">
                ...
                <div>
                    ...
                    <p class="text-2xl font-black text-slate-800">{{ $completedAssignments }} / {{ $totalAssignments }}</p>
                </div>
            </div>
        </div>
            ...
            <div class="flex items-center">
                ...
                <div>
                    ...
                    <p class="text-2xl font-black text-slate-800">
                        {{ $averageGrade >= 50 ? 'Good Standing' : 'Needs Improvement' }}
                    </p>
                </div>
            </div>
        </div>
                    ...
                    @foreach($courseProgress as $course)
                        <div>
                            <div class="flex justify-between mb-2">
                                <span class="text-sm font-bold text-slate-700 uppercase">{{ $course['name'] }}</span>
                                <span class="text-sm font-bold text-emerald-600">{{ $course['progress'] }}%</span>
                            </div>
                            <div class="w-full bg-slate-100 rounded-full h-3">
                                <div class="bg-emerald-500 h-3 rounded-full" style="width: {{ $course['progress'] }}%"></div>
                            </div>
                        </div>
                    @endforeach
                </div>
                ...
                <div class="space-y-4">
                    @forelse($upcomingDeadlines as $deadline)
                        <div class="border-l-4 border-emerald-400 pl-4">
                            <p class="text-xs text-emerald-300 font-bold uppercase">{{ $deadline->due_at->format('M d • h:i A') }}</p>
                            <p class="text-sm font-medium">{{ $deadline->title }}</p>
                        </div>
                    @empty
                        <p class="text-sm text-emerald-200 italic">No upcoming deadlines.</p>
                    @endforelse
                </div>
