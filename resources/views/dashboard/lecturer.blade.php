    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div class="bg-indigo-600 p-6 rounded-3xl shadow-lg shadow-indigo-200 flex items-center text-white">
            ...
            <div>
                ...
                <p class="text-3xl font-black">{{ $courses->count() }}</p>
            </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-3xl shadow-sm border border-indigo-100 flex items-center">
            ...
            <div>
                ...
                <p class="text-3xl font-black text-gray-800">{{ $ungradedItems }}</p>
            </div>
        </div>
    </div>
...
<div class="lg:col-span-2 space-y-6">
    <div class="bg-white p-8 rounded-3xl shadow-sm border border-indigo-50">
        <div class="flex justify-between items-center mb-6">
            <h3 class="text-xl font-bold text-gray-800">Assessment Calendar</h3>
            <a href="{{ route('lecturer.assignments.index') }}" 
               class="text-sm font-bold text-indigo-600 hover:text-indigo-700 flex items-center">
                View All →
            ...
            @forelse($upcomingAssignments as $assignment)
                <div class="group flex items-center p-4 rounded-2xl border border-transparent hover:border-indigo-100 hover:bg-indigo-50/50 transition-all duration-300">
                    <div class="bg-indigo-600 text-white p-3 rounded-xl font-bold text-center min-w-[60px]">
                        <span class="block text-xs uppercase opacity-80">{{ \Carbon\Carbon::parse($assignment['due_at'])->format('M') }}</span>
                        <span class="text-lg">{{ \Carbon\Carbon::parse($assignment['due_at'])->format('d') }}</span>
                    </div>
                    <div class="ml-6">
                        <p class="text-sm font-bold text-indigo-600">Due by {{ \Carbon\Carbon::parse($assignment['due_at'])->format('h:i A, d M Y') }}</p>
                        <h4 class="font-bold text-gray-900">{{ $assignment['title'] }}</h4>
                        <p class="text-sm text-gray-500">{{ $assignment['course_name'] }}</p>
                    </div>
                </div>
            @empty
